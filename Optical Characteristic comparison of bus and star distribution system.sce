//Program to form comparison showing to the channel loss in an optical fiber against number of
nodes for Bus Distribution System and Star Distribution System
clear ;clc ; close ;
// Given da ta
...
N =0:0.01:11;
Cl_bus =2* alpha_cr +( N -1) * alpha_fc * L_bu +(2* alpha_cr + L_ac ) *( N -3) +(2* alpha_cr +
L_tr ) + L_sp + alpha_cr ;
Hm =abs( Cl_bus );
figure ;
plot2d (N , Hm ,2) ;
